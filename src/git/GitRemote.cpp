...
GitRemote::GitRemote(url u, git_direction dir) {
  ...
  if (error < 0) {
    cerr<<"Could not create detached remote:"<<endl<<git_error_last()->message<<endl;
    exit(1);
  }
  ...
  if (error < 0) {
    cerr<<"Could not connect to detached remote:"<<endl<<git_error_last()->message<<endl;
    exit(1);
  }
}
...
GitRemote::GitRemote(git_repository *repo, git_direction dir) {
  ...
  if (error < 0) {
    cerr<<"Could not lookup remote:"<<endl<<git_error_last()->message<<endl;
    exit(1);
  }
  ...
  error = git_remote_connect(remote, dir, &callbacks, NULL, NULL);
  if (error < 0) {
    cerr<<"Could not connect to remote:"<<endl<<git_error_last()->message<<endl;
    exit(1);
  }
}
